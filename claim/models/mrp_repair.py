...
class MrpRepair(models.Model):
    ...
    claim_id = fields.Many2one('claim', readonly=True)
    ...
    def create_picking(self, isInMove, isPieces):
        ...
        if isPieces:
            ...
            if self.product_id.type == 'product':
                ...
                move._action_assign()
                # move_line = self._create_stock_move_line(picking, move)
                # move.write({'move_line_ids': [(6, 0, [move_line.id])]})
                # picking.write({'move_line_ids': [(6, 0, [move_line.id])]})
    ...
    def _compute_stock_picking(self, picking, mrp_repair_line_id):
        ...
        move = move.filtered(lambda x: x.state not in ['done', 'cancel'])._action_confirm()
        move._action_assign()
        # move_line = self._create_stock_move_line_pieces(picking, move, mrp_repair_line_id)
        # move.write({'move_line_ids': [(6, 0, [move_line.id])]})
        # picking.write({'move_line_ids': [(6, 0, [move_line.id])]})
        # picking.action_assign()
    ...
    def action_repair_end(self):
        ...
        for repair in self:
            ...
            vals = {
                ...
                'move_id': super(MrpRepair, self).action_repair_done().get(repair.id)
            }
            repair.write(vals)
        return True
